...
* **Resolved by A1, mechanism superseded 2026-08-14** — whether certification is per-transaction or batch-level: still moot for this role in the sense that certification isn't this role's assigned responsibility. [ui/screens/internal-certification-queue.md](ui/screens/internal-certification-queue.md) documents certification as per-record, with no bulk action — that screen is now reconciled against the unified model (issue #50), and its no-bulk-action design is unchanged by that reconciliation.
...
**Corrected 2026-08-15** — this section previously claimed "four survive as genuinely open" while listing five items under "Still open," two of which (items 1 and 2) already showed as resolved in their own text — an inconsistency present before today. A6's confirmation resolves item 3 as well, and the Dashboard rework (issue #50) resolves item 7. One item remains genuinely open: item 6.
...
3. ~~SLA for Trustee action on routed developer requests?~~ **Resolved by A6** (client decision, 2026-08-15) — the source's two-number reading (waiting time vs. delivery time) is confirmed correct; no new SLA figure is needed.
...
5. ~~Transaction certification: per-transaction or batch?~~ **Moot per A1, mechanism superseded 2026-08-14** — this was a question about the Auditing Bureau Officer's certification cadence; the role does not certify, by practice rather than by scope restriction now. The per-record, no-bulk-action cadence in [ui/screens/internal-certification-queue.md](ui/screens/internal-certification-queue.md) is unchanged by that screen's reconciliation against the unified model (issue #50).
7. ~~Does the Institution Relationship Manager get an institution-wide oversight dashboard?~~ **Resolved 2026-08-15** — the reworked [ui/screens/dashboard.md](ui/screens/dashboard.md) is institution-wide by design, visible identically to every role, not just the Institution Relationship Manager. This exceeds what the original question asked (a dashboard *for the IRM*) by making the same institution-wide view available to everyone, consistent with A4's confirmation that no service or function is role-specific. Answer A5's High-confidence position — that institution-wide visibility should exist at all — is what's actually vindicated here, not a role-scoped implementation of it.
