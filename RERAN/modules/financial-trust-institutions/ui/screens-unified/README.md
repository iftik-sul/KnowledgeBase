...
**Why this direction, not the reverse:** the open question was never really "wizard vs. single-page" as a style preference — it was whether the per-service field matrix survives as one generic layout at all. A full audit of all eighteen services' Section 6 (Required Information) found it doesn't: **13 services need a flat field layout (Pattern A), 2 need a repeatable-group layout (Pattern B — Sale Procedure/Heirs #13, Split Ownership #16), and 1 needs a field-selection-then-conditional-pairs layout (Pattern C — Update Title Deed Information #15).** Two more (Company Shares Sale #14, Contract Cancellation #18) have plural, unstructured fields the source doesn't disambiguate and are defaulted to Pattern A pending a product decision. See [submit-application.md](submit-application.md)'s own Section 3 for the full per-service breakdown.

That finding applies identically to a single-page form — `service-request.md` needed the same three-pattern fix and never got it. The choice between the two screens therefore came down to which layout handles the underlying complexity better, not which needed less work:
...
2. **Pattern B and Pattern C are both easier to build and use as a dedicated step.** A repeatable "add another heir" control, or a checklist-then-conditional-value-pairs interaction, gets cluttered embedded in the middle of a long single-page form; a step gives each room to be its own focused interaction.
...
| `service-request.md` | [submit-application.md](submit-application.md) | **Resolved.** `service-request.md` deleted; Submit Application is canonical. |
...
Every screen named in issue #50's original scope is now resolved. Nothing remains open on this issue.
