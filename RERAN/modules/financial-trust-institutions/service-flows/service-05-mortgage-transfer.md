...
derived_from:
  - "RERAN/reference/source-of-truth/RERAN_service_flows_v2.md" (row 32)
...
The **Mortgage Transfer** service is a Group C service defined by row 32 of the master service table. This document carries forward the row’s workflow, channel, output and SLA, while marking design detail not enumerated by the source as proposed.

## 2. Purpose

Provide a controlled and auditable route to complete mortgage transfer and issue the source-specified output after regulatory review and settlement.

## 3. Description

The source cross-references mortgage registration. Expanded: customer prepares transfer requirements; the bank employee enters documents; the bank checker and Department audit; the fee is deducted from the bank account; outputs are emailed. It differs by validating the existing and receiving mortgage parties.
...
> **Proposed** — row 32 says to submit or enter documents but does not enumerate them. This working list is analogous to the individual-user template and remains client data for confirmation.
...
2. The applicant selects **Mortgage Transfer**, enters required data and submits documents.
...
1. The workflow, channel, output and SLA are sourced from row 32.
...
7. No statutory window or SLA beyond row 32 is asserted.
