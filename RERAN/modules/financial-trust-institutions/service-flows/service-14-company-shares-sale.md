...
derived_from:
  - "RERAN/reference/source-of-truth/RERAN_service_flows_v2.md" (row 41)
...
The **Company Shares Sale** service is a Group C service defined by row 41 of the master service table. This document carries forward the row’s workflow, channel, output and SLA, while marking design detail not enumerated by the source as proposed.

## 2. Purpose

Provide a controlled and auditable route to complete company shares sale and issue the source-specified output after regulatory review and settlement.

## 3. Description

Customer uses a Trustee Centre, submits documents, has the transaction entered, pays fees and receives outputs by email.
...
> **Proposed** — row 41 says to submit or enter documents but does not enumerate them. This working list is analogous to the individual-user template and remains client data for confirmation.
...
2. The applicant selects **Company Shares Sale**, enters required data and submits documents.
...
**Source workflow detail:** Customer uses a Trustee Centre, submits documents, has the transaction entered, pays fees and receives outputs by email.
...
1. The workflow, channel, output and SLA are sourced from row 41.
...
7. No statutory window or SLA beyond row 41 is asserted.
