...
derived_from:
  - "RERAN/reference/source-of-truth/RERAN_service_flows_v2.md" (row 37)
...
The **Finance Lease Release** service is a Group C service defined by row 37 of the master service table. This document carries forward the row’s workflow, channel, output and SLA, while marking design detail not enumerated by the source as proposed.

## 2. Purpose

Provide a controlled and auditable route to complete finance lease release and issue the source-specified output after regulatory review and settlement.

## 3. Description

The source cross-references finance-lease registration. Expanded: the operator assists the customer, submits release documents, enters the transaction, takes the payment step and output is emailed; the identified lease is discharged rather than created.
...
> **Proposed** — row 37 says to submit or enter documents but does not enumerate them. This working list is analogous to the individual-user template and remains client data for confirmation.
...
2. The applicant selects **Finance Lease Release**, enters required data and submits documents.
...
**Source workflow detail:** The source cross-references finance-lease registration. Expanded: the operator assists the customer, submits release documents, enters the transaction, takes the payment step and output is emailed; the identified lease is discharged rather than created.
...
1. The workflow, channel, output and SLA are sourced from row 37.
...
7. No statutory window or SLA beyond row 37 is asserted.
