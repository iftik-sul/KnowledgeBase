...
derived_from:
  - "RERAN/reference/source-of-truth/RERAN_service_flows_v2.md" (row 39)
...
The **Grant Property Mortgage** service is a Group C service defined by row 39 of the master service table. This document carries forward the row’s workflow, channel, output and SLA, while marking design detail not enumerated by the source as proposed.

## 2. Purpose

Provide a controlled and auditable route to complete grant property mortgage and issue the source-specified output after regulatory review and settlement.

## 3. Description

Customer prepares mortgage requirements from the bank; a bank employee enters documents via the Online Mortgage System; the bank auditor audits it; it is sent to the Department; fees are deducted from the bank account; outputs are delivered by email.
...
> **Proposed** — row 39 says to submit or enter documents but does not enumerate them. This working list is analogous to the individual-user template and remains client data for confirmation.
...
2. The applicant selects **Grant Property Mortgage**, enters required data and submits documents.
...
**Source workflow detail:** Customer prepares mortgage requirements from the bank; a bank employee enters documents via the Online Mortgage System; the bank auditor audits it; it is sent to the Department; fees are deducted from the bank account; outputs are delivered by email.
...
1. The workflow, channel, output and SLA are sourced from row 39.
...
7. No statutory window or SLA beyond row 39 is asserted.
