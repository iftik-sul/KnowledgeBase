...
derived_from:
  - "RERAN/reference/source-of-truth/RERAN_service_flows_v2.md" (row 34)
...
The **Finance Lease Registration** service is a Group C service defined by row 34 of the master service table. This document carries forward the row’s workflow, channel, output and SLA, while marking design detail not enumerated by the source as proposed.

## 2. Purpose

Provide a controlled and auditable route to complete finance lease registration and issue the source-specified output after regulatory review and settlement.

## 3. Description

Customer uses a Trustee Centre; the operator submits documents, enters the transaction, takes the payment step and the output is emailed. Under the working position, the operator is using the same online service on the customer’s behalf.
...
> **Proposed** — row 34 says to submit or enter documents but does not enumerate them. This working list is analogous to the individual-user template and remains client data for confirmation.
...
2. The applicant selects **Finance Lease Registration**, enters required data and submits documents.
...
**Source workflow detail:** Customer uses a Trustee Centre; the operator submits documents, enters the transaction, takes the payment step and the output is emailed. Under the working position, the operator is using the same online service on the customer’s behalf.
...
1. The workflow, channel, output and SLA are sourced from row 34.
...
7. No statutory window or SLA beyond row 34 is asserted.
