...
**Found 2026-08-16, not resolved.** This module's Escrow Request Queue (Feature #4) treats all six developer-originated escrow request types (Services #8–#12, #20–#21 from real-estate-developer) as one queue with one status vocabulary (`Awaiting Assessment → Under Assessment → Certified`). Real-estate-developer's own docs for the same transactions split across two features with two different, non-matching vocabularies: `feature-04-escrow-management.md` (`Pending Approval → Under Review → Approved → Released`, for #8/9/20/21) and `feature-05-fund-release-request.md` (a 9-stage tracker including `Under Bank Review → Under RERA Review`, for #10/12). Neither developer-side term appears in this module's vocabulary.

**Separately, this feature's own status flow has a gap**: `Certified` is shown as the terminal status, but this feature's own prose says a certified request is then "forwarded to RERA's escrow department for final audit" — no status here represents that subsequent RERA review, even though real-estate-developer's `Under RERA Review` (its Feature #5) shows that stage genuinely exists on the other side of the same transaction.

Written independently across both modules without cross-checking — exactly the kind of drift the project's merge-conflict discipline exists to catch. Not fixed here; needs a client or architecture decision on which vocabulary, if either as-is, should be authoritative. See `feature-04-escrow-request-queue.md`'s own Feature Overview for the full detail, and the mirrored flag in real-estate-developer's `shared-platform-features.md`.
...
Open gap (not counted above): Staff Management — documented responsibility, no screen, no feature. Cross-module flag (not counted above): Escrow status vocabulary mismatch against real-estate-developer.
