...
**Corrected again 2026-08-15 — #5 was misclassified as Pattern C, found in a later audit pass still.** Checking #5's own Processing Workflow directly (not just its category-level description) found no second user session anywhere in the file: "New Owner Information" is data the transferor enters about the new owner, not a separate login/notification/accept-or-decline step performed by that new owner — unlike #6, #8, #10, #11, and #14, all of which were checked directly and do have a genuine second session. #5 moved to Pattern B. It does keep one genuine complication none of Pattern B's other members have: a dual online/Trustee-Centre channel with different payment timing (`payments.md` Category 3), noted inline below rather than treated as its own pattern, since the field shape itself — the thing patterns actually classify — is ordinary Pattern B.

| Pattern | Shape | Services |
| :---- | :---- | :---- |
| **A — Search/Lookup** | Search criteria only; no documents, no confirmation step | #1, #2, #3 |
| **B — Standard single-applicant** | Applicant + Property + transaction-specific flat fields | #4, #5 (dual channel — see note above), #17, #18, #20, #22, #41, #43 (provisional — see note above) |
| **C — Two-party with counterparty confirmation** | Primary applicant fills and submits; a named second party receives a notification and must separately confirm before the application proceeds | #6, #8, #9, #10, #11, #14 |
...
4. **#43's pattern is provisional (Pattern B), pending client confirmation of whether an exchange needs two-sided confirmation** — added 2026-08-15. If confirmed two-sided, it moves to Pattern C and needs the same counterparty-confirmation treatment as #6/#9–#11/#14, including a second user session and the Counterparty Confirmation Card component.
