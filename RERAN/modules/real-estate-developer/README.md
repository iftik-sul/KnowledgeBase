...
| 6 | [Register Sale Associated with an Initial Mortgage](service-flows/service-06-register-mortgage-linked-sale.md) | Real Estate Development *(now validates against Financial & Trust Institutions' Mortgage Registration — see that service's own file)* |
...
| 10 | [Project Profit Withdrawal](service-flows/service-10-withdraw-project-profit.md) | Real Estate Development *(UI mismatch resolved 2026-08-16 — now its own feature, [#13](service-flows/feature-13-profit-withdrawal-request.md); no screen built yet)* |
...
**Not yet done:** cross-referencing each of these 27 files back to whichever shared-platform feature it belongs to (decided needed, 2026-08-16 — see Open Questions).

## Shared Platform Features

* [shared-platform-features.md](shared-platform-features.md) — **rebuilt 2026-08-16** from this module's actual 19 built screens, replacing an earlier draft that copied individual-user's generic Submit/Track/Respond/Resubmit framing without checking it against what's here. This module has no single canonical submission form — what's actually built is **13 features, all written**: one cross-cutting **Applications** tracker, six **domain workspaces** (Projects, Property Registrations, Escrow Management, Fund Release Request, Profit Withdrawal Request, Sales & Disclosures) that applications actually originate from, and six **General Platform** features (Dashboard, Documents, Company Profile, Reports, Notifications, Help & Support). **Sales & Disclosures resolved 2026-08-16** as a compliance-tracking layer parallel to Property Registrations, not a domain workspace tied to a numbered service, and **confirmed the same day to require a disclosure for every sale, not only some. Fund Release Request split 2026-08-16**: it previously covered both Service #10 and Service #12, but the client confirmed Service #10 (a profit distribution) never fit the screen's construction-milestone shape — Service #10 now has its own feature, #13, with no screen built yet. **Proposed — not sourced, needs client confirmation.**

## UI Specifications

* [ui/README.md](ui/README.md) — screen index (19 distinct screens), all reachable by all four roles. **Does not yet include a screen for Feature #13 (Profit Withdrawal Request)** — none exists; the feature document proposes a field list without a built UI behind it.
...
* **Resolved 2026-08-16** — `sales-and-disclosures.md`'s relationship to the numbered service catalogue. It is not tied to any numbered service; it is a compliance-tracking layer parallel to Property Registrations. See [Feature #6](service-flows/feature-06-sales-and-disclosures.md). **Also resolved the same day**: a disclosure is required for every sale registered via Property Registrations, not only some — by client decision.
* **Resolved 2026-08-16** — Feature #5 (Fund Release Request)'s UI mismatch: Service #10 (Project Profit Withdrawal) is confirmed **not** a fit for the milestone/construction-draw-shaped screen. Split into its own feature, [#13](service-flows/feature-13-profit-withdrawal-request.md) — no screen built yet for it; the field list is proposed, not designed against a real UI.
* **Resolved 2026-08-16** — Feature #9 (Company Profile)'s missing Validation Summary is confirmed not a gap: this is a profile-editing screen, not a submission service, so no pre-submission validation panel is expected.
* **New 2026-08-16, unresolved.** All 27 individual service-flow files should be updated to cross-reference whichever shared-platform feature they belong to — decided needed, not yet executed. Tracked as a standalone sweep.
* **New 2026-08-16, unresolved.** Service #6 now validates its mortgage reference against Financial & Trust Institutions' Service #3 in real time — see that service's own file for the full five-decision design (validated, requires `Completed`, reject-after not warn-before, real-time not batch, invisible on FTI's side).
