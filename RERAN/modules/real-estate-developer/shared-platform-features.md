...
**13 of 13 features are now written (2026-08-16).**

## Application Lifecycle (7)
...
* [Feature #5 — Fund Release Request](service-flows/feature-05-fund-release-request.md) — Service #12 only, following the 2026-08-16 split. **Previously covered Service #10 too, until the client confirmed a genuine mismatch** — see Feature #13.
* [Feature #6 — Sales & Disclosures](service-flows/feature-06-sales-and-disclosures.md) — **resolved 2026-08-16, not a domain workspace tied to a numbered service.** Checked directly: Services #1 and #6 both cite Property Registrations in their own `derived_from`, not this screen — so this is a **compliance-tracking layer** (sale record + separate disclosure obligation, two lifecycles) running parallel to Property Registrations, the same shape as financial-trust-institutions' Compliance Reports feature. **Confirmed 2026-08-16, by client decision: a disclosure is required for every Property Registrations sale**, not only some — closing this feature's own previously-open question.
* [Feature #13 — Profit Withdrawal Request](service-flows/feature-13-profit-withdrawal-request.md) *(new, 2026-08-16, by client decision)* — Service #10, split out of Feature #5. The client confirmed Fund Release Request's construction-verification shape (Engineer Certificate, Quantity Surveyor Report, milestone percentage) never fit a profit distribution. **No screen exists** — proposed here, sibling to Feature #5, reusing its sourced approval chain and status vocabulary while replacing the mismatched construction fields with entitlement-basis fields.
...
| :---- | :---: | :---- |
| Application Lifecycle (Applications + 5 domain workspaces + Profit Withdrawal Request) | 7 | Written |
| General Platform | 6 | Written |
| **Total** | **13** | **13 of 13 written** |
...
* **This module's Fund Release Request (Feature #5) and Profit Withdrawal Request (Feature #13)** — both use the same sourced vocabulary (`Trustee Review`, `RERA Escrow Audit`), inherited by #13 from the split rather than re-derived.
...
See Features #4, #5, and #13's own Feature Overview sections, and financial-trust-institutions' `feature-04-escrow-request-queue.md`, for the full detail.
...
**Decided 2026-08-16, in five steps, by client decision.** Service #6 (Register Sale Associated with an Initial Mortgage) validates its mortgage reference against financial-trust-institutions' Service #3 (Mortgage Registration), in real time, requiring a `Completed` match, with no pre-submission warning and no visibility on FTI's side. Full detail in Service #6's own file and FTI's Service #3 file — this is a service-level dependency, not a shared-feature one, so it isn't repeated in full here.
...
1. Does the full 13-feature structure correctly represent the module's shared layer? Needs client confirmation.
2. ~~Sales & Disclosures' relationship to Property Registrations~~ **Decided 2026-08-16 — required for every sale.**
3. ~~Fund Release Request's UI-mismatch question~~ **Resolved 2026-08-16 — split into Feature #13.**
4. ~~Company Profile's missing Validation Summary~~ **Decided 2026-08-16 — not applicable to a profile page.**
5. **Decided 2026-08-16 — should.** Individual service-flow files (Services #1–27) should be updated to cross-reference whichever domain-workspace or Applications feature they belong to. Not yet executed — a 27-file sweep, tracked separately.
6. **New 2026-08-16.** No screen exists for Feature #13 (Profit Withdrawal Request) — the field list is proposed, not designed against a real UI. Needs design review before being built.
