...
The **Register Sale Associated with an Initial Mortgage** service allows a developer to register the provisional sale of a unit where the purchaser is financing the purchase through a mortgage. The application captures the mortgage institution, reference number, and amount, and — **by client decision, 2026-08-16** — validates that reference against financial-trust-institutions' Mortgage Registration records, in real time, immediately upon submission, before the application can proceed to RERA's audit. The matched FTI record must be `Completed` — its mortgage registration must have already passed both of FTI's own gates (internal certification, then RERA's audit) before this sale can validate. **No pre-submission warning is shown** — a developer submits without any check against FTI's records first, and learns the outcome of that check within the same request, via the automatic return described in Section 13.

> **Corrected 2026-08-16, five decisions.** (1) The previously-asserted "coordinating... linking" relationship to FTI's Service #3 was an unconfirmed inference, corrected to reflect the sourced workflow's actual self-contained design. (2) Decided the service should validate the mortgage after all. (3) Decided the FTI record must be `Completed`. (4) Decided the UX is reject-after, not warn-before. (5) **Decided the validation call is real-time and synchronous** — the check against FTI's records happens immediately as part of the submission request, not on a delay or batch schedule; a developer whose mortgage isn't `Completed` gets the automatic return back right away, not after an unexplained wait. This closes the design entirely: **despite this service's name — "Initial Mortgage" — the two applications cannot be filed concurrently**, and the developer finds that out immediately upon trying.

## 2. Purpose

Provide a regulated, provisional record of a mortgage-financed unit sale, validated in real time against a mortgage the financing institution has already fully registered, under a single project unit.

## 3. Description

The developer selects the property, records the purchaser and mortgage-institution details, attaches supporting documents, selects a payment method, and submits online — with no prior indication of whether the cited mortgage has finished FTI's own process. The system immediately validates the cited mortgage reference against financial-trust-institutions' Mortgage Registration records as part of that same submission, requiring the matched record to already be `Completed`, before the application proceeds to RERA's review. RERA reviews and issues a Mortgage Provisional Registration Certificate and an Electronic Map.
...
* Mortgage Reference Number *(validated in real time against financial-trust-institutions' Mortgage Registration records — must match a `Completed` record; see Section 12)*
...
**Yes — before RERA's decision, and before mortgage validation.** The source workflow places payment at the point of submission, ahead of any review. Payment is collected before the mortgage-validation check runs — a developer whose mortgage isn't yet `Completed` will have already paid by the time the real-time check returns a failure. Paid per transaction through the shared platform payment gateway.
...
**6 business days**, assuming the matching FTI mortgage record is already `Completed` at submission — mortgage validation itself adds negligible time, since it's a real-time call within the same submission request, not a queued or scheduled check. Where the FTI record isn't yet `Completed`, the developer receives an automatic return immediately (Section 13) and must resubmit once it completes — this 6-day figure does not include any such wait, since there's no pending/holding state built into the design; see Business Rules.
...
↓
Submit Application Online *(no prior check against FTI's mortgage status)*
↓
**System Validates Mortgage Reference Against Financial & Trust Institutions' Mortgage Registration Records, in Real Time, Within the Same Request — Requires a `Completed` Match** *(decided 2026-08-16)*
↓
*If Not Found, Mismatched, or Not Yet `Completed`:* Application Automatically Returned to Developer, Immediately *(see Section 13)*
↓
...
Submitted
↓
Validating Mortgage Reference *(real-time, synchronous, within the same request as submission — decided 2026-08-16; not a developer-facing waiting state, since it resolves before the submission response returns)*
↓
Under Review
↓
Approved
...
* Returned *(includes automatic system-generated returns for a mortgage reference not found, mismatched, or not yet `Completed` on financial-trust-institutions' side, returned immediately as part of the same submission request, in addition to RERA's own manual returns — the application's own return reason should distinguish system-generated returns from RERA's, and should specifically say "mortgage not yet completed" when that is the cause. This is a terminal outcome for the current submission, not a waiting state — no re-check or pending-retry mechanism exists; the developer resubmits as a fresh action once the mortgage completes.)*
* Rejected
* Cancelled
...
* Application Returned *(RERA decision, or immediate automatic mortgage-validation failure — including "not yet Completed")*
...
* Financial & Trust Institutions Service #3 – Mortgage Registration *(cross-module dependency, decided 2026-08-16: this service validates its mortgage reference against FTI Service #3's records in real time, requiring a `Completed` match, before proceeding to RERA's audit — see Sections 5, 12, 13. **The practical sequencing this creates: the institution's mortgage registration must finish first, and the developer finds this out immediately upon submitting and paying, not after a delay.** This is a product decision, not a sourced requirement. FTI Service #3's own file cites this service back, exposes a corresponding real-time lookup endpoint, and stays invisible to the dependency on its own side — see that file's Feature Overview.)*
...
* **Validate Mortgage Registration** *(cross-module, real-time synchronous call to Financial & Trust Institutions' Mortgage Registration records, made within the submission request itself — decided 2026-08-16. Checks the cited Mortgage Reference Number exists, is `Completed`, and its institution/amount are consistent. Called only after submission and payment, never before.)*
* Upload Documents
...
* Mortgage reference is validated against financial-trust-institutions' Mortgage Registration records in real time, within the submission request, and must match a `Completed` record, before the application proceeds to RERA's review.
* This check happens only after submission and payment, never before.
* An application whose mortgage reference cannot be validated — not found, mismatched, or not yet `Completed` — is automatically and immediately returned to the developer, with a reason distinguishing it from a RERA-issued return and specifically flagging "not yet completed" where that is the cause.
...
5. No pre-submission check or warning is shown to the developer regarding the cited mortgage's FTI status. Validation happens only after submission and payment; a failed validation is a terminal return for that submission, not a pending or retryable state — the developer resubmits fresh once the mortgage completes.
6. **Decided 2026-08-16.** The validation call in Rule 3 is real-time and synchronous — made within the same submission request, not deferred to a batch or scheduled process. The developer receives the outcome (proceed, or automatic return) immediately, not after an unexplained delay.
7. Payment must be completed before the application proceeds for regulatory review, and is collected before the mortgage-validation check runs.
8. Every application receives a unique application reference number.
9. All submissions, approvals, payments, and notifications must be permanently recorded in the audit trail.
...
3. ~~Real-time or batch validation?~~ **Decided 2026-08-16 — real-time, synchronous, within the submission request.**
...
All open questions raised by the mortgage-validation dependency are now decided.
