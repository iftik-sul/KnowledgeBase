...
| **E+F** | Individual User | 41 | 6 | 4 | 43 service flows written; count reconciled; `payments.md`, `open-questions.md`, `navigation.md`, and `role-workflows.md` added 2026-08-15 (analysis layer complete); no UI |
...
**Pattern worth naming, corrected 2026-08-15 (twice).** This section originally said "no module has both stages complete and merged yet," then was corrected to name **Group C** as the first module with service flows and UI both complete and merged. Both framings are now superseded: **Group B is equally complete and was merged first** — PR #36 landed its 27 service flows on `main` on 2026-08-11, under a UI that was already finished, and issue #58 has since reconciled both layers against the unified-access and per-transaction payment decisions. Group C's 18 flows and 18 screens are reconciled against the same two decisions. So there are now two fully round-tripped modules, not one, and the "drafted-but-unmerged" description Group B carried here was stale for four days before this correction. Individual User still has full flows and no UI at all. Group C remains the cleaner house-style example — its derivation ran source → flows → UI in order, where Group B's ran backwards.
...
**Status, updated 2026-08-15.** The analysis layer is now complete: `payments.md`, `open-questions.md`, `navigation.md`, and `role-workflows.md` were added, built by checking all 43 service-flow files individually against the master table rather than trusting a module-wide pattern. That check surfaced two findings of note beyond this module's own scope:

* A **systemic payment-timing documentation defect**, not a payment-model question — see the Cross-Cutting Observations section below, which this finding now feeds into alongside the existing Group B and Group C exceptions.
* A **genuine three-way role-attribution conflict** on Register/Renew Lease (Services #23/#24) between the master table (names Tenant), the current service-flow files (frame it as Landlord-only), and this module's own `roles-and-responsibilities.md` (gives both roles overlapping "register/renew lease" responsibilities, unresolved). Flagged for client confirmation in `open-questions.md` A1, the same standard Group C's A4 needed before role-specific claims in this module can be trusted.

**Still open:** the 43 service-flow files themselves have not yet been corrected to match `payments.md`'s findings — `open-questions.md` B2 lists every affected file. This is recommended as the immediate next piece of work, before UI work starts, since `navigation.md` and `role-workflows.md` were both built against the *corrected* payment timing, not the currently-published Section 9 lines, meaning the module's own documents now disagree with each other until that correction pass happens.
...
**4. Individual User UI.** The mirror image of #3 — flows exist, screens don't. Closing this and #3 leaves the project with two fully round-tripped modules (source → flows → UI) to use as the house style for everything after. **Group C now already is one** (see the Landscape table) — Individual User closing this gap would make it the second, not the first. **Updated 2026-08-15:** the Individual User service-flow files need their own Section 9 payment-timing correction pass (see that module's `open-questions.md` B2) before this step starts, or the UI will be built against the wrong timing for at least fifteen services.
...
| Section 9 payment-timing boilerplate wrong in ~15 of 39 files | Individual User | Open — new 2026-08-15, see `open-questions.md` B2. Recommended before UI work starts (Proposed Sequence item 4). |
| Register/Renew Lease role attribution unresolved (Tenant vs Landlord) | Individual User | Open — new 2026-08-15, see `open-questions.md` A1. Flagged for client confirmation. |
...
**Corrected a fifth time, 2026-08-15 — Individual User was the third module checked, and it broke this claim in a new way none of the first four corrections anticipated.** Every correction so far had been about *where in the sequence* payment falls relative to RERA's decision. Individual User's audit (`payments.md`, `open-questions.md` B2) found something structurally different: **the module's own service-flow files assert a uniform "pay before submission" rule in their Section 9 field that is simply wrong against their own cited source, for at least fifteen of thirty-nine files** — not a payment-model choice the client made differently, a documentation defect where the same boilerplate line was applied to every file regardless of what its own source row said. Individual User's Service #28 (Request Rental Valuation, row 84) is the cleanest single exception found on the whole platform so far — the source states outright, with no ordering to infer, "Pay fees after approval." Individual User also holds **five genuinely no-fee services** (#17, #18, #33, and one branch each of #7 and #42), the same shape as Group C's Service #2 and Group B's (apparently mis-documented) Service #19 — meaning at least three of the platform's eight modules now have a confirmed or suspected no-fee exception, not one.

**The actual pattern, as verified as of 2026-08-15 across Groups B, C, and Individual User: payment timing is not uniform and cannot be inferred from a neighbouring service, a category, or a module — and in at least one module, it cannot even be trusted from that module's own documentation without checking the underlying source row directly.** Roughly two-thirds of fee-bearing services pay before a decision; at least eight pay after; one pays twice; several carry no fee at all. Groups D, H, and A remain entirely unaudited on this question. Given this observation has now been wrong in five successive ways — never checked against other modules; then checked against other modules but not against every service within Group C itself; then corrected for Group C's edges but not its own internal exceptions; then corrected for all of Group C while still asserting a platform-wide conclusion with Group B entirely unaudited; then corrected for Group B while assuming every module's own documentation could be trusted at face value, which Individual User's audit disproved — treat any future "no exception remains" or "the documentation is accurate" claim on this topic with real skepticism until it's been checked service-by-service, against the source directly, not module-by-module or file-by-file.
