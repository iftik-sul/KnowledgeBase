...
**First post-merge audit pass, 2026-08-15.** Found and corrected two gaps the correction pass had introduced: (1) Section 13 (Application Status Flow) in #9–#16, #23, #24, and #26 still showed only the online channel's upfront payment sequence, with no status reflecting the Trustee Centre / Service Center channel's audit-then-pay timing that Section 9/12 already documented for the same eleven files — an "Audited — Awaiting Payment" status was added to each (#27 checked, needed no change). (2) The module's own top-level `README.md` had never been updated to index the analysis layer or UI package, still reflecting the module's 2026-08-10 state. Also found, not fixed: `roles-and-responsibilities.md` remains unedited since 2026-08-09 and still gives Landlord and Tenant verbatim-overlapping responsibilities — now tracked as `open-questions.md` D1, flagged to the client rather than silently edited.

**Second post-merge audit pass, 2026-08-15, same day.** The first pass's own fix (adding "Audited — Awaiting Payment" to the service-flow files) had introduced a new inconsistency: the UI package's `status-badges.md`, written *before* that fix, still described one uniform status ("Approved — Awaiting Payment") covering #28 and the counter-channel path of #9–#16/#23/#24/#26/#27 — a description that predated and never got reconciled against the fix. Checking directly against the files found three separate errors compounding from that one stale description: #28 genuinely reaches full approval before payment (that status name is correct only there); #9–#16/#23/#24/#26 pay at an earlier checkpoint, right after audit, which is exactly why the first pass gave it a *different* name; and #27 uses neither status at all — its post-review payment step is just the standard `Payment Pending` repositioned later in its own sequence. `application-details.md` had additionally, and separately, listed **#5** as needing this action — wrong, since #5's counter channel pays *before* approval, a fact already established and correctly excluded from `status-badges.md`'s own list, just never carried over to `application-details.md`'s parallel one. Split into two correctly-scoped statuses in `status-badges.md`, then propagated the same fix into every other file that had copied the original description: `application-details.md`, `components.md`, `dashboard.md`, `applications.md`, `notifications.md` — six files total, all tracing to the same single root cause.

**Status:** the most complete module in the project — service flows, analysis layer, and UI, all built, merged, and internally consistent with each other, including two post-merge audit passes. One standing item remains client-facing: `open-questions.md` D1. The pattern worth naming from both audit passes: correcting a source-of-truth file (the service-flow files) does not automatically propagate to every downstream document that cites it by name — this has now happened three times in this module's history (`services-overview.md`, `module-roadmap.md` itself, and now six UI files at once), and is worth treating as a standing risk on any future correction that touches something multiple other documents reference.
...
| ~~UI package's `status-badges.md`, `application-details.md`, `components.md`, `dashboard.md`, `applications.md`, `notifications.md` still described one uniform "Approved — Awaiting Payment" status, stale against the fix above (#5 wrongly included, #27 wrongly grouped in, #9–#16/#23/#24/#26 mislabeled)~~ | Individual User | **Resolved 2026-08-15**, found in a second post-merge audit pass, same day. Split into two correctly-scoped statuses across all six files. See the Group E+F profile above. |
