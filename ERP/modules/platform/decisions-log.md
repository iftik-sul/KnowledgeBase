...
## D17 — Sourced qty is decoupled from the Order Line's need; all received stock stays linked to the order

**Decision:** `qty_to_source` on a Sourcing Order is not capped at the Order Line's need — a company can order more or less than a line currently requires (vendor MOQs, bulk pricing, buffer, or a short vendor shipment). All received stock stays fully linked to the Order Line, regardless of quantity — there is no automatic split into a reserved portion and an unreserved surplus portion.
**Alternatives considered (superseded):** An earlier version of this decision split received stock automatically — only `min(received_qty, remaining_need)` counted as "reserved" to the line, with any excess auto-releasing to general, unreserved stock. That version is superseded by this one.
**Rationale:** Real purchasing doesn't work in exact quantities — vendor minimums, bulk pricing, and partial shipments are normal, not exceptional. Rather than computing a reserved/surplus split automatically, it's simpler and more transparent to keep everything received against a Sourcing Order linked to its Order Line, whatever the quantity, and let the *status* — not a quantity split — communicate whether the line is covered. This is also more predictable for the person using the system: "how much came in" and "is this order done" are two separate, simple questions, rather than one calculated field.

## D18 — Sourcing completion is a direct comparison, not a reservation calculation

**Decision:** A Sourcing Order exposes `qty_required` (what the Order Line needed at creation) and `received_qty` (total received, fully linked). `sourcing_status` is derived directly: **Complete** if `received_qty ≥ qty_required`, **Not Completed** if `received_qty < qty_required`.
**Alternatives considered (superseded):** Track fulfillment via a computed `qty_reserved_to_line` field, treating anything beyond the line's need as automatically-released surplus stock.
**Rationale:** A direct comparison is easier to reason about and display than a computed reservation split, and matches how the business actually wants to see it: is this order's sourcing done, yes or no. Any qty beyond what's required simply stays linked to the order as extra — if that extra ever needs to move to general stock for a different order, that's a manual action (unchanged from the reservation principle in the original D17, retained here even though the automatic split it applied to has been dropped).
