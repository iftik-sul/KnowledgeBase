...
Twenty-two baseline requirements, plus two decisions that amend them ([3I-DEC-009](/3i/decisions/dec-009-seats-as-account-pool.md), [3I-DEC-010](/3i/decisions/dec-010-waiver-covers-all-seats.md)) — both scope changes requiring §21.3 sign-off, listed in the [decision register's scope-changes table](/3i/decisions/README.md#scope-changes-against-srd-v20).
...
| Additional seats | Per-seat monthly charge — **price not yet confirmed by client** |
...
| Waivers | Four fixed tiers — 25%, 50%, 75%, 100%, applied to the whole subscription including additional seats |
...
This module owns the **Subscription** and its seat **quantity** as a Stripe line item. It does not own the **Learner** record or its activation state — that lives in `identity-and-access`, which this module reads and writes through the seat-purchase and seat-cancellation flows. See [data-model.md](data-model.md) for the split.

A waiver discounts the **whole subscription**, additional seats included — a family of six on a 100% waiver receives a genuinely zero invoice ([3I-DEC-010](/3i/decisions/dec-010-waiver-covers-all-seats.md)).
...
| Item | Blocks | Note |
| :---- | :---- | :---- |
| Per-seat price | Checkout, Stripe price object creation | Client dependency #1, §22.2 — priority ask |
...
No requirement in this module is otherwise unresolved. The three items above are inputs this module needs, not gaps in the specification itself.

## Change Requests Owed to the Client

Two decisions in this module change SRD v2.0 rather than interpret it — the seat-as-permanent-grant model and waiver-covers-all-seats. Both are part of the consolidated change request under §21.3 — full list in [decisions/README.md](/3i/decisions/README.md#scope-changes-against-srd-v20).
