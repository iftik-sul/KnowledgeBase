...
- On submit, computed age determines the registration path: under 13 blocks (FR-AUTH-03), 13–17 adds the guardian fields (FR-AUTH-05), 18+ proceeds normally.
- **The under-13 block fires client-side for immediate feedback, and is re-validated server-side before any account record is created.** Client-side validation alone is not the control — FR-AUTH-04's hashed-attempt logging happens server-side regardless of what the client showed.
...
- Guardian email (FR-AUTH-05) uses the same format validation but has no uniqueness constraint — one guardian may be named on multiple 13–17 registrations.
...
- No composition restriction (no "cannot be 0000" rule) unless a rate-limiting decision later makes weak PINs a meaningful risk — not currently required.
...
- Only a Member whose own account date of birth indicates 18+ can reach the profile creation form (FR-FAM-01). A 13–17 standalone account never sees the "Add profile" affordance at all — it is absent, not disabled, consistent with the treatment of the DOB field above.
