...
model Account {
  ...
  dateOfBirth     DateTime  // FR-AUTH-01/02. Must resolve to 18+ or the row is never created.
...
On **first** login via Google/Apple: capture `dateOfBirth` before any Account row is written, then run the exact same age branch as `/auth/register` above — two outcomes only, blocked or created. **No exception for this path** — the acceptance criterion is explicit that under-18 cannot succeed by any route.
