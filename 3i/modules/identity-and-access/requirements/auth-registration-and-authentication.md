...
| **FR-AUTH-03** | Registration is **blocked** where DOB indicates under 18. The block message is neutral and does not disclose the threshold in a way that invites retry |
...
**On FR-AUTH-03's neutral message.** The requirement is that the block does not teach the user how to defeat it. Stating the exact age threshold invites an amended birth year; a neutral refusal does not. FR-AUTH-04 catches the retry either way, but the message should not be the thing that prompts it. The copy must read naturally for anyone under 18, from a young child's guardian to a near-adult teenager. **This message is exempt from AI translation** — [3I-DEC-019](/3i/decisions/dec-019-safeguarding-strings-exempt-from-ai-translation.md) — since a mistranslation either invites a retry or reads as an accusation, in languages the build team cannot check.
...
| ID | Requirement |
| :---- | :---- |
| **FR-AUTH-11** | Device allowance is **seats plus two, floor of three** per account. Device swap permitted twice per 30 days. Devices are visible and de-authorisable by the Member |
| **FR-AUTH-12** | Concurrent video streams are limited to the number of purchased learner seats, minimum one |

**FR-AUTH-11's device cap scales with seats.** [3I-DEC-015](/3i/decisions/dec-015-device-allowance-scales-with-seats.md): one seat allows three devices, six seats allows eight. This keeps every purchased seat usable — a family buying five seats can use them from five-plus devices, avoiding an unenforceable term under Australian Consumer Law (FR-REF-05 already acknowledges those guarantees apply regardless of policy terms).
...
These are the sign-off conditions under §21.2.

1. A DOB of under 18 cannot produce an account **by any route, including social login**.
2. **A device registration beyond the account's current allowance is refused** with a clear message and a link to device management.
3. An unverified account is blocked from enrolment, checkout, and chat.
