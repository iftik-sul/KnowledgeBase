...
- **Open items (2026-09-26, CL-047).** The "Open items created by this decision" list above is **stale and superseded by the live register, OL-OPN-001.** Closed since: **item 3** (SMS gateway) → Alpha SMS, CL-036; **item 6** (API framework) → TypeScript/Node, ADR-003, with the RLS-baseline half still open as register item S0-7; **item 7** (Apple Developer account timing) → an account exists, registered personally, and the question is now the entity decision, register item N-1. Items 1, 2, 4, 5 and 8 remain open and are tracked in the register. The list above is preserved as accepted; **the register is authoritative.**
