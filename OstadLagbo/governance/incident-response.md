...
3. **Preserve:** place a legal hold (OL-RET-001, ADM-18 — `POST /v1/admin/accounts/{id}/legal-hold` with a written reason; before Slice 4, a direct database update) on both accounts' data — profiles, the chat thread, offer history, identity documents. Do this before anything else can expire or purge.
