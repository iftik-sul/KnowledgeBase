...
**Shagred sent-offers list:** `offer where shagred_account_id = :me`, ordered `created_at desc`, each with live `status` and, for pending, `expires_at` (client renders "N days remaining"). **Ostad received-offers list:** `offer where ostad_account_id = :me`, pending first ordered `expires_at asc` (soonest-to-expire on top), then resolved. **Chat list (both roles):** `chat_thread where :me ∈ participants`, ordered `last_message_at desc`, each with the counterpart's display name (the live profile where it exists; a counterpart who is **departed — self-deleted or terminated (`deleted_at` set) — or purged** shows to the other participant as **"deleted account"** per OFR-06/CL-032) and an unread count (`chat_message where read_at is null and sender_account_id ≠ :me`).

**Snapshot vs. live display (clarification, 2026-09-25).** The `shagred_display_name_snapshot` / `ostad_display_name_snapshot` on `connection` back the connection **record** — keeping it readable for admin, history (SGP-DM), and analytics after a party purges. They are **not** the live inbox/chat display shown to the *other participant*: once a counterpart **departs — self-deletes or is terminated (`deleted_at` set) — or purges**, that participant sees them as **"deleted account"** (OFR-06, CL-032), not the snapshot name. The two surfaces serve different readers.
