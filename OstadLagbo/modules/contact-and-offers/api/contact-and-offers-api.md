...
|---|---|---|---|---|
| `POST /v1/offers` | A registered **Shagred** whose **own profile is complete** (OFR-01/SGP-05: an Ostad must see at least a name, Thana, and District before deciding — `shagred_profile.profile_complete`); Ostads cannot initiate; accepts `Idempotency-Key`. Policy checks, all at insert: recipient satisfies the discoverability predicate, is **not paused** (OSP-11), and has **no block** either way (OFR-01) | `{ ostad_id, message }` — free text, ≤500 chars | `{ id, status: "pending", expires_at }` — a `pending` offer; sending grants the Ostad visibility of the Shagred's profile (SGP-05) and pushes "offer received" | `validation_failed` (message empty or >500); `forbidden` (caller is an Ostad); `state_conflict` (`reason: "profile_incomplete"` — the caller's Shagred profile lacks a name or complete address; the client routes them to setup, SGP api; or `reason: "ostad_not_accepting"` — recipient paused, public state); `conflict` (`reason: "duplicate_pending_pair"` or `reason: "pending_limit_reached"`); `not_found` (no such approved Ostad, **or a block exists** — opacity rule) |
...
**No `system` messages (CL-030).** The platform never writes into the chat stream on block, suspension, unblock, or reinstatement — a message is only `text` or `voice`. A freeze is conveyed by the thread's frozen state and a neutral client banner (OFR UI), never by an inserted message, so nothing about a block or a moderation action ever appears in the shared history (RNT-08 opacity).
...
The failed-send `state_conflict` reason and the header/inbox `frozen_reason` are **neutral** for a block or a counterpart suspension (`unavailable` — the client shows "You can no longer message this person" and never attributes it to a block or a moderation action, RNT-08); `deleted` shows only when the counterpart's account is gone (OFR-06). The blocker alone learns of their own block, from their block list. A **block freeze is stored** (`chat_thread.block_frozen_at`) and is **not** lifted by unblocking (RNT-08, CL-023) — the old thread stays frozen, and messaging resumes only after a new accepted offer forms a new connection. Suspension freeze clears on reinstatement (predicate); a deletion-frozen thread stays frozen permanently (the tombstone status never changes).
