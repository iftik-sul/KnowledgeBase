...
| division_id / district_id / thana_id | uuid → `admin_area` (ADM) | Thana + District to permitted Ostads; Division implied | The only locational data (SGP-02) |
...
| purge_at / anonymized / legal_hold | per Overview conventions | — | Retention behavior below |
...
| ostad_profile_id | uuid, nullable → ostad_profile | Nulled when the Ostad's account is purged; the row remains, displayed as "deleted account" |
...
|---|---|---|
| shagred_profile | Row marked `anonymized` at day 30: display_name → "Former Shagred", photo_ref/gender/address/postal nulled. The row persists only as long as something references it (reviews per RNT-05, connections per Overview rule 2); when nothing does, it purges | — |
